defmodule DurationFormatter do
  def format_duration(seconds) do
   year = div(seconds, 31536000)
   seconds_rem = seconds - year * 31536000
   ...
   str = Enum.map(0..Enum.count(numbers)-1, fn(x) ->
     cond do
      Enum.at(numbers, x) > 0 ->
       Kernel.inspect(Enum.at(numbers, x)) <> " " <> Enum.at(words, x) <>
       cond do
         Enum.at(numbers,x) > 1 ->
           "s"
         true->
   ...
   |> Enum.intersperse(", ")
   str = Enum.map(0..Enum.count(str)-1, fn x ->
    cond do
