...
string multiply(string a, string b) {
  ...
  for (int i = alen - 1; i >= 0; i--) {
    for (int j = alen - 1 - i; j > 0; j--) ret[i].append("0");
  }
  int c = alen - 1;
  ...
  for (string::iterator itA = a.end() - 1; itA >= a.begin(); itA--) {
    ...
    if (resto) ret[c].insert(0, to_string(resto));
    c--;
    resto = 0;
  }
  ...
  if (restoant) ret1.insert(0, to_string(restoant));
  remove_zeros(ret1);

  if (ret1.length() == 0) return "0";
