...
class RSUProgram:
    ...
    def get_funs(self, tokens, scope="-1"):
        if (scope, re.findall(r"p\d+", ''.join(tokens))[0]) in self.functions:
            raise Exception('Function already defined in this scope')
        self.functions[(scope, re.findall(r"p\d+", ''.join(tokens))[0])] = ''.join(re.findall(r"(?!p\d+[^q]+q)", ''.join(tokens[1][-1])))
