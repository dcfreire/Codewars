...
class RSUProgram:
    ...
    def get_funs(self, tokens, scope="-1"):
        ...
        fun = re.findall(r"(^p\d+|.+q)", ''.join(tokens))
        ...
        if c != -1:
            self.get_funs(re.findall(
                r"([FRL)]\d*|\(|[pP]\d+|q)", fun[1][c:-1]), scope + "|" + fun[0][1:])
    ...
    def convert_to_raw(self, tokens, scope='-1', fn='-1'):
        ...
        while tokens:
            if re.match(r"[RLF]\d+", tokens[0]):
                ...
                tokens[-1:-1] = re.findall(r"([FRL)]\d*|\(|[pP]\d+|q)", self.functions[(saux, tokens[0][1:])])
                del tokens[0]
...
RSUProgram("""
p10FP10Fqp21p12FP10LRqFFFqFFFP10
