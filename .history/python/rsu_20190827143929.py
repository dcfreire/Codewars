...
class RSUProgram:
    ...
    def get_tokens(self, source=self.source):
        source = re.sub(
            r"((\/\/.*\n?)|((\/\*)((\*\/){0}[^\6])*?(\*\/){1}))", " ", source)
        if re.findall(r"[a-zA-Z)]\s\d", source):
            raise Exception('Invalid indentation/spacing')
        if re.findall(r"[^FRL()Ppq0-9\s]", source):
            raise Exception('Operation not defined')
        if re.findall(r"[^FRL)Pp\d]\d+", source):
            raise Exception('Invalid integer location')
        source = re.sub(r"\s", "", source)
        if re.findall(r"(p|P)([^0-9]|$)", source):
            raise Exception('Function without an indentifier')
        if re.findall(r"\D0\d+", source):
            raise Exception('Leading zeroes')

        if re.findall(r"(p|P)[^\d]+", source):
            raise Exception('Function without an identifier')
        return re.findall(r"([FRL)]\d*|\(|[pP]\d+|q)", source)
