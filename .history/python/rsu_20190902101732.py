            ...
            if re.search(r"p\d+", tokens[c]):
                ...
                while bn:
                    ...
                    if tokens[c][0] == 'p':
                        ...
                        bn -= 1
                    c += 1
                if (scope, tokens[c][1:]) in self.functions:
                    raise Exception('Function already defined in this scope')
                self.functions[(scope, tokens[c][1:])] = tokens[bc+1:c-1]
                
                self.get_funs(tokens[bc+1:c-1], scope + "|" + tokens[c][1:])
                del tokens[bc:c]
                c = bc - 1
            c += 1
    stacktrace = []
...
FFLFFRFFFRFFFRFFFFFLFFFFFLFFFFFLFFFRFFFRFFFFFLFFFFFLFFFFFLFFFRFFFRFFFFFLFFFFFLFFFFFLFFFRFFFRFFFFFLFFFFFLFFFFFL
FR
FFFLFFRFFFRFFFRFFFFFLFFFFFLFFFFFLFFFRFFFRFFFFFLFFFFFLFFFFFLFFFRFFFRFFFFFLFFFFFLFFFFFLFFFRFFFRFFFFFLFFFFFLFFFFFL
FP2
FR
""")
